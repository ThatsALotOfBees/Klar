...
export const api = {
  ...
  // Pull the current tunnel/server URL from the host's GitHub repo. The
  // host's `up` command auto-writes client-releases/server.json with
  // whatever URL the tunnel claimed, so this lets the client adapt when
  // the URL has changed since the EXE was built. Best-effort — on any
  // failure (no network, no updateRepo set, etc.) we fall back to the
  // bundled URL silently. Returns the resolved URL or null.
  discoverServerUrl: async () => {
    const cfg = (typeof window !== 'undefined' && window.KLAR_CONFIG) || {};
    if (!cfg.updateRepo) return null;
    const branch = cfg.updateBranch || 'main';
    // Cachebust by date so GitHub Raw's CDN doesn't serve a stale value.
    const url = `https://raw.githubusercontent.com/${cfg.updateRepo}/${branch}/client-releases/server.json?t=${Date.now()}`;
    try {
