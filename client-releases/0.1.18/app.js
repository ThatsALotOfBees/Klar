...
class CallManager {
  ...
  // Start sharing the screen / a window. opts: { height, fps }.
  async startScreenShare(opts = {}) {
    ...
    const constraints = {
      video: {
        width:     { ideal: Math.round(height * (16 / 9)), max: Math.round(height * (16 / 9)) },
    ...
    try {
      ...
      // User cancelled the picker, or OS denied. Not an error worth toasting.
      klog.info('call.screen.start', 'cancelled', { err: e.message });
      return false;
    }
...
function showLocalScreenShareIndicator() {
  const bar = _callBanner();
  if (!bar) return;
  let pill = bar.querySelector('[data-screen-pill]');
  if (!pill) {
    pill = document.createElement('span');
    pill.className = 'screen-share-pill';
    pill.dataset.screenPill = '1';
    pill.textContent = 'SHARING';
    const meta = bar.querySelector('.call-banner-meta');
...
function hideLocalScreenShareIndicator() {
  const bar = _callBanner();
  if (!bar) return;
  const pill = bar.querySelector('[data-screen-pill]');
  if (pill) pill.remove();
}

let _remoteScreenViewer = null;
function showRemoteScreenShare(stream, peer, onClose) {
  hideRemoteScreenShare();
  const v = document.createElement('div');
  v.className = 'remote-screen-viewer';
  v.innerHTML = `
    <div class="rsv-header">
      <span class="rsv-name"></span>
      <button class="rsv-fullscreen" type="button" title="Fullscreen">⛶</button>
      <button class="rsv-close" type="button" title="Hide">×</button>
    </div>
    <video autoplay playsinline></video>
  `;
  v.querySelector('.rsv-name').textContent = (peer && (peer.displayName || peer.username)) || 'Screen share';
  const video = v.querySelector('video');
  video.srcObject = stream;
  video.muted = true; // audio (if any) flows through the audio track separately
  v.querySelector('.rsv-close').addEventListener('click', () => {
    hideRemoteScreenShare();
    if (typeof onClose === 'function') onClose();
  });
  v.querySelector('.rsv-fullscreen').addEventListener('click', () => {
    if (document.fullscreenElement) document.exitFullscreen().catch(() => {});
    else video.requestFullscreen().catch(() => {});
  });
  document.body.appendChild(v);
  _remoteScreenViewer = v;
}
function hideRemoteScreenShare() {
  if (_remoteScreenViewer) { try { _remoteScreenViewer.remove(); } catch {} _remoteScreenViewer = null; }
}

// Popover with resolution + fps picker. Anchored above the screen-share
// button. Confirms with "Start sharing" which calls callMgr.startScreenShare.
function openScreenSharePopover(anchorBtn) {
  // Toggle: if already open, close it.
  const existing = document.querySelector('[data-screen-popover]');
  if (existing) { existing.remove(); return; }

  const ss = state.settings.screenShare;
  const pop = document.createElement('div');
  pop.className = 'screen-popover';
  pop.dataset.screenPopover = '1';
  pop.innerHTML = `
    ...
    <div class="sp-section">
      <div class="sp-label">Frame rate</div>
      <div class="sp-row" data-sp-fps></div>
    </div>
    <div class="sp-actions">
      <button class="ghost" data-sp-cancel>Cancel</button>
      <button data-sp-start>Share screen</button>
    </div>
    <p class="muted small" style="margin: 8px 0 0">Pick a window or your full screen in the system picker.</p>
  `;
  const resRow = pop.querySelector('[data-sp-resolutions]');
  ...
  const fpsRow = pop.querySelector('[data-sp-fps]');
  ...
  pop.querySelector('[data-sp-cancel]').addEventListener('click', () => pop.remove());
  pop.querySelector('[data-sp-start]').addEventListener('click', async () => {
    pop.remove();
    const ok = await callMgr.startScreenShare({ height: ss.height, fps: ss.fps });
    if (!ok) klog.info('call.screen', 'share start declined');
  });
  ...
  const onDown = (e) => { if (!pop.contains(e.target) && e.target !== anchorBtn) { pop.remove(); cleanup(); } };
