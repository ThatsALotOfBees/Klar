...
async function boot() {
  ...
  // Discover the *current* server URL from GitHub. If the host's tunnel
  // landed on a different subdomain than what's bundled into this EXE,
  // this gets us the fresh URL with no manual reconfig. Best-effort and
  // silent — if it fails we fall back to the bundled value.
  await api.discoverServerUrl();

  // Connectivity probe first. If we can't reach the configured server,
  // there's no point rendering the auth form — show the "couldn't connect"
  // screen instead so the user has a clear message and a retry button.
  const probe = await api.probe();
  if (!probe.reachable) {
    renderServerUnreachable(probe);
    return;
  }

  // If the user opted in to "stay logged in" before, restore the session.
  // We verify it's still valid by calling /api/me; if the token has been
  // revoked or the server lost the session, we drop the saved blob and
  ...
  if (saved && saved.token) {
    ...
    try {
      ...
      rememberUser(user);
      // Refresh the bundle in storage in case the server rewrote anything.
