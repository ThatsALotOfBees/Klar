...
// ICE servers: STUN candidates from multiple providers (so the call doesn't
// die if one is unreachable from your network), plus a free TURN relay
// (Open Relay Project) for the cases where neither peer can punch a hole
// through their NAT. Without TURN, calls behind symmetric / strict NAT
// stay forever in "connecting" — which is exactly the bug we keep hitting.
const ICE_CONFIG = {
  iceServers: [
    { urls: [
  ...
  // 'all' lets ICE try host -> reflexive -> relay candidates in order. If
  // the call gets stuck before reaching the relay, switching this to
  // 'relay' (forces TURN) is the easiest debug step.
...
class CallManager {
  constructor() {
    ...
    this.startedAt = 0;
    this._pendingCandidates = []; // unused with perfect negotiation; kept for compat
    this._dialingAudio = null; // outgoing-call ring tone
    this.screenStream = null;
    this.screenSender = null;
    // Peer state mirror — populated by call.state messages from the other
    // side (peer broadcasts whenever they mute/deafen/start sharing).
    this.peerMicMuted = false;
    this.peerDeafened = false;
    this.peerSharing  = false;
    // Speaking indicator state (Web Audio analyser + RAF loop).
    this._speakCtx = null;
    this._speakLoop = 0;
  ...
  // Tell the peer we just changed our mic / deafen / screen-share state
  // so they can update their indicators (and so the receiver-side mute
  // badge isn't stuck on the wrong value).
  broadcastState() {
  ...
  _startSpeakingDetection() {
    ...
    try {
      this._speakCtx = new (window.AudioContext || window.webkitAudioContext)();
    } catch { return; }
    const tick = () => {
      if (!this._speakCtx) return;
      // Self
      ...
      // Peer (remote audio is on this.remoteAudio.srcObject — but Web
      // Audio sources from MediaStream work even when the audio is also
      // playing through an <audio> element)
      ...
      const speakingFor = (an) => {
        if (!an) return false;
        const buf = new Uint8Array(an.frequencyBinCount);
        an.getByteFrequencyData(buf);
        let sum = 0; for (const v of buf) sum += v;
        const avg = sum / buf.length;
        return avg > 18; // tunable threshold
      };
      const bar = _callBanner();
      if (bar) {
        const selfTile = bar.querySelector('[data-ptile-self]');
        const peerTile = bar.querySelector('[data-ptile-peer]');
        if (selfTile) selfTile.classList.toggle('speaking', !state.globalMicMuted && speakingFor(this._selfAnalyser));
        if (peerTile) peerTile.classList.toggle('speaking', !this.peerMicMuted && speakingFor(this._peerAnalyser));
      }
  ...
  async startCall(peer, dmId) {
    ...
    this._startDialing();
    // Don't create the WebRTC offer yet — we wait until the callee sends
    // accept. Sending the offer immediately races the callee's own
    // _buildPeer() inside accept() and leaves the call permanently stuck
    // in "Connecting…".
  }
  ...
  async accept() {
    ...
    // Just signal accept — DON'T build the peer yet. The caller will see
    // the accept and send us an offer; we build our peer in onSignal(offer)
    // right before applying the remote description. Building before the
    // offer arrives caused both sides to race onnegotiationneeded and send
    // simultaneous offers (glare), which left calls forever stuck in
    // "Connecting…" because the impolite peer ignored the polite peer's
    // offer.
  ...
  async onIncomingInvite(msg) {
    ...
    // Audio cue: same dialing.mp3 looping so the callee gets a continuous
    // ring (matches the caller's experience), plus a one-shot notify
    // cue + taskbar flash for cases where the window is buried.
    playNotifySound();
    this._startDialing();
  ...
  async onAccept(msg) {
    ...
    // Caller's job now: build the peer connection, create the offer, send
    // it. Callee will build THEIR peer when our offer arrives (deferred
    // build pattern — avoids both sides firing onnegotiationneeded in
    // parallel and glaring).
    try {
      await this._buildPeer();
      const offer = await this.pc.createOffer({ offerToReceiveAudio: true });
      await this.pc.setLocalDescription(offer);
      klog.info('call.offer', 'sent', { call: this.callId });
      this._sendSignal('signal', { kind: 'offer', sdp: this.pc.localDescription });
    } catch (e) {
  ...
  async onSignal(msg) {
    ...
    try {
      if (payload.kind === 'offer') {
        ...
        // Callee builds the peer lazily when the offer arrives — that way
        // only the CALLER ever fires the initial onnegotiationneeded, so
        // no glare. (Once both sides finish the initial handshake, we
        // wire up perfect-negotiation for renegotiation events like
        // screen-share track adds.)
        if (!this.pc) {
          klog.info('call.offer', 'received, building peer', { call: this.callId });
          await this._buildPeer();
        }
        // Perfect-negotiation glare check, in case this offer is a
        // renegotiation that collides with our own. During initial setup
        // we won't have made an offer (we're impolite/callee), so this
        // never trips.
        const offerCollision = this._makingOffer || this.pc.signalingState !== 'stable';
        const ignoreOffer = !this._isPolite && offerCollision;
        if (ignoreOffer) {
          klog.info('call.signal', 'glare offer ignored (impolite)', { state: this.pc.signalingState });
          return;
        }
        await this.pc.setRemoteDescription(description);
        // Drain any candidates that arrived before the description was set.
        ...
        this._sendSignal('signal', { kind: 'answer', sdp: this.pc.localDescription });
      } else if (payload.kind === 'answer') {
        if (!this.pc) {
          klog.warn('call.signal', 'answer with no pc, dropping');
          return;
        }
        klog.info('call.answer', 'received', { call: this.callId, state: this.pc.signalingState });
        ...
        if (!this.pc || !this.pc.remoteDescription || !this.pc.remoteDescription.type) {
          // Buffer until we've applied the remote description.
          (this._pendingCandidates ||= []).push(payload.candidate);
          return;
        }
        try { await this.pc.addIceCandidate(payload.candidate); }
        catch (e) { klog.warn('call.ice', 'addIceCandidate failed', { err: e.message }); }
      }
    } catch (e) {
      klog.error('call.signal', 'apply failed', { kind: payload.kind, err: e.message, stack: e.stack });
    }
  }
  ...
  // and applied by toggleGlobalMic / toggleGlobalDeafen. CallManager just
  // reads them when the localStream / remoteAudio first come up.
  ...
  async _buildPeer() {
    ...
    // Insert a real-time noise gate (Web Audio) when the slider is non-zero.
    // Reads state.settings.voice.noiseGate live in the loop, so dragging
    // the slider during a call changes the threshold immediately.
    if (state.settings.voice.noiseGate > 0) {
      try {
        ...
        this._micGateLoop = setInterval(() => {
          ...
          // Smooth a touch so we don't click on transitions.
    ...
    // Perfect-negotiation state. Used ONLY for renegotiation (e.g. someone
    // adds a screen-share track mid-call), not for the initial handshake.
    // The polite peer rolls back its own offer on glare; the impolite one
    // wins. Caller = polite (arbitrary but consistent).
    this._isPolite = !!this.isCaller;
    this._makingOffer = false;
    this._initialHandshakeDone = false;
    this._pendingCandidates = [];
    ...
    this.pc.ontrack = (e) => {
      ...
      if (track.kind === 'audio') {
        ...
        // Peer is sharing their screen — show it in the floating viewer.
        showRemoteScreenShare(e.streams[0], this.peer, () => {
          // No local action when the user closes the viewer; the remote
          // user is still sharing. We just hide our window.
        });
        track.onended = () => {
    ...
    // Renegotiation handler — fires when tracks are added/removed AFTER
    // the initial handshake (typically from screen-share toggling). The
    // event also fires once during initial setup because addTrack(audio)
    // sets the negotiation-needed flag — we suppress that case so only
    // the caller's manual createOffer (in onAccept) drives the handshake.
    this.pc.onnegotiationneeded = async () => {
      if (!this._initialHandshakeDone) {
        klog.info('call.negotiate', 'suppressed during initial handshake');
        return;
      }
      try {
        ...
        klog.error('call.negotiate', 'failed', { err: e.message });
      } finally {
        this._makingOffer = false;
      }
    };

    // Mark the initial handshake done the first time the signaling state
    // returns to 'stable' after we've applied an SDP. Renegotiation
    // (via onnegotiationneeded) becomes safe from that point on.
    this.pc.onsignalingstatechange = () => {
      if (!this.pc) return;
      const s = this.pc.signalingState;
      klog.info('call.sig.state', s, { call: this.callId });
      if (s === 'stable' && !this._initialHandshakeDone &&
          this.pc.localDescription && this.pc.remoteDescription) {
        this._initialHandshakeDone = true;
        klog.info('call.handshake', 'initial done, renegotiation enabled', { call: this.callId });
      }
    };
    ...
    this.pc.onicecandidate = (e) => {
      if (e.candidate) {
        ...
        // Log candidate type so we can diagnose connectivity issues.
        // host = on-network LAN; srflx = STUN-discovered public IP;
        // relay = TURN. If we never see srflx or relay, NAT traversal
        // is broken and the call will hang on connecting.
    ...
    this.pc.onconnectionstatechange = () => {
      ...
      if (s === 'connected') {
        ...
        // Initial state push so the peer sees our mute/deafen badge.
        this.broadcastState();
  ...
  // Start sharing the screen / a window. opts: { height, fps, sourceId? }.
  // When sourceId is provided (Electron path), we use Chromium's legacy
  // chromeMediaSource constraints — bypassing the (flaky-on-Win10) display
  // media picker entirely. Without sourceId we fall back to the standard
  // getDisplayMedia path (browser tab / Mac / etc.).
  async startScreenShare(opts = {}) {
    ...
    let stream;
    if (opts.sourceId) {
      // Electron / chromeMediaSource path: deterministic, returns the
      // chosen window/screen directly. Constraints are the legacy form
    ...
    // Cap encoder bitrate so even 1440p stays bandwidth-friendly. Numbers
    // tuned to land below typical home upload (~5–10 Mbps).
    ...
    // The browser/OS shows a "stop sharing" indicator; if the user clicks
    // it (track ends), tear down on our side too.
