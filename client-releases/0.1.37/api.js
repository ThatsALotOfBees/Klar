...
// Manual user override from Settings → Advanced → "Override server URL".
// Takes precedence over discoverServerUrl(). Set via api.setManualServerUrl().
let _manualServerUrl = null;
export function setManualServerUrl(url) {
  _manualServerUrl = url ? url.replace(/\/+$/, '') : null;
}
...
function serverBase() {
  if (_manualServerUrl) return _manualServerUrl;
...
export const api = {
  ...
  me: () => request('/api/me'),
  updateMe: (patch) => request('/api/me', { method: 'PATCH', body: JSON.stringify(patch) }),
  searchUsers: (q) => request(`/api/users/search?q=${encodeURIComponent(q)}`),
  listBlocks:  () => request('/api/blocks'),
  blockUser:   (userId) => request(`/api/users/${userId}/block`,   { method: 'POST' }),
  unblockUser: (userId) => request(`/api/users/${userId}/block`,   { method: 'DELETE' }),

  // Friendships
  listFriends:   () => request('/api/friends'),
  requestFriend: (username) => request('/api/friends/request', { method: 'POST', body: JSON.stringify({ username }) }),
  acceptFriend:  (userId) => request(`/api/friends/${userId}/accept`, { method: 'POST' }),
  ...
  createGroupChat:   (body) => request('/api/group-chats', { method: 'POST', body: JSON.stringify(body) }),
  getGroupChat:      (id) => request(`/api/group-chats/${id}`),
  renameGroupChat:   (id, name) => request(`/api/group-chats/${id}`, { method: 'PATCH', body: JSON.stringify({ name }) }),
  addGroupMembers:   (id, userIds) => request(`/api/group-chats/${id}/members`, { method: 'POST', body: JSON.stringify({ userIds }) }),
  removeGroupMember: (id, userId) => request(`/api/group-chats/${id}/members/${userId}`, { method: 'DELETE' }),
  listGroupMessages: (id, before) => request(`/api/group-chats/${id}/messages${before ? `?before=${before}` : ''}`),
  sendGroupMessage:  (id, content, clientId, attachments) =>
    request(`/api/group-chats/${id}/messages`, {
      method: 'POST',
      body: JSON.stringify({ content, clientId, attachments: attachments || [] }),
    }),
  deleteGroupMessage: (id, msgId) => request(`/api/group-chats/${id}/messages/${msgId}`, { method: 'DELETE' }),
  ...
  sendMessage: (dmId, content, clientId, attachments) =>
    request(`/api/dms/${dmId}/messages`, {
      method: 'POST',
      body: JSON.stringify({ content, clientId, attachments: attachments || [] }),
    }),
  deleteMessage: (dmId, msgId) => request(`/api/dms/${dmId}/messages/${msgId}`, { method: 'DELETE' }),
  ...
  // Channels
  createChannel: (serverId, name, kind, categoryId) =>
    request(`/api/servers/${serverId}/channels`, {
      method: 'POST',
      body: JSON.stringify({ name, kind: kind || 'text', categoryId: categoryId || null }),
    }),
  updateChannel: (channelId, patch) => request(`/api/channels/${channelId}`, { method: 'PATCH', body: JSON.stringify(patch) }),
  deleteChannel: (channelId) => request(`/api/channels/${channelId}`, { method: 'DELETE' }),

  // Categories
  createCategory: (serverId, name) =>
    request(`/api/servers/${serverId}/categories`, { method: 'POST', body: JSON.stringify({ name }) }),
  updateCategory: (categoryId, patch) =>
    request(`/api/categories/${categoryId}`, { method: 'PATCH', body: JSON.stringify(patch) }),
  deleteCategory: (categoryId) =>
    request(`/api/categories/${categoryId}`, { method: 'DELETE' }),
  listChannelMessages: (channelId, before) => request(`/api/channels/${channelId}/messages${before ? `?before=${before}` : ''}`),
  sendChannelMessage: (channelId, content, clientId, attachments) =>
    request(`/api/channels/${channelId}/messages`, {
      method: 'POST',
      body: JSON.stringify({ content, clientId, attachments: attachments || [] }),
    }),
  deleteChannelMessage: (channelId, msgId) => request(`/api/channels/${channelId}/messages/${msgId}`, { method: 'DELETE' }),

  // Resolve a third-party URL to OpenGraph metadata (videoUrl, thumbnail,
  // title). Used by the embed pipeline for medal.tv etc — sites whose
  // direct video URL isn't visible from the message text but is published
  // as og:video on the page.
  resolveEmbed: (url) => request('/api/embed/resolve', {
    method: 'POST',
    body: JSON.stringify({ url }),
  }),

  // Generic single-file uploader. The endpoint defaults to /api/uploads
  // (server-stored) but image/video files default to /api/uploads/catbox
  // so they end up on a public CDN — much faster + zero load on the host
  // server's bandwidth, and the catbox URL stays valid even if the host
  // moves. Caller can force one endpoint via `endpoint` opt.
  uploadFile: (file, onProgress, opts = {}) => {
    let endpoint = opts.endpoint;
    if (!endpoint) {
      const t = (file.type || '').toLowerCase();
      const isMedia = t.startsWith('image/') || t.startsWith('video/');
      endpoint = isMedia ? '/api/uploads/catbox' : '/api/uploads';
    }
    return new Promise((resolve, reject) => {
      const xhr = new XMLHttpRequest();
      xhr.open('POST', apiUrl(endpoint));
      xhr.setRequestHeader('Content-Type', file.type || 'application/octet-stream');
      xhr.setRequestHeader('X-Klar-Filename', encodeURIComponent(file.name || 'file'));
      xhr.setRequestHeader('bypass-tunnel-reminder', 'klar-app');
...
export class Realtime extends EventTarget {
  ...
  // Send a raw JSON message over the WebSocket. Used by the WebRTC call
  // code to relay SDP / ICE / call-control via the existing connection.
  // Returns true if the send was attempted; false if the socket is not
  // currently open (caller should treat as transient).
  send(obj) {
