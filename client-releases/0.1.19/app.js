...
function openSettingsModal() {
  openModal('tpl-modal-settings', async (modal) => {
    ...
    // ---- Hotkeys (in Voice tab) ----
    const hkList = modal.querySelector('[data-hotkey-list]');
    if (hkList) {
      const labels = { mute: 'Toggle mute', deafen: 'Toggle deafen', hangup: 'Leave call', screen: 'Toggle screen share' };
      const renderHotkeys = () => {
        hkList.innerHTML = '';
        for (const action of Object.keys(labels)) {
          ...
          btn.textContent = comboToLabel(settings.hotkeys[action]);
          btn.addEventListener('click', () => {
            ...
            const onKey = (e) => {
              if (e.key === 'Escape') {
                ...
                const combo = comboFromEvent(e);
                if (!combo) return; // bare modifier — wait
                e.preventDefault(); e.stopPropagation();
