...
class CallManager {
  ...
  async accept() {
    ...
    try {
      // Build the peer FIRST, then send accept — that way when the caller
      // receives accept and sends their offer, we already have a pc ready
      // and can apply the remote description without a race.
  ...
  async onAccept(msg) {
    ...
    // Now that the callee has accepted (and built their pc — they do that
    // before sending accept), we build OURS. Adding the audio track in
    // _buildPeer triggers onnegotiationneeded, which creates the offer and
    // ships it. The answer arrives via onSignal. (No manual createOffer
    // here; the perfect-negotiation handler handles initial AND
    // renegotiation in one place.)
    try {
      await this._buildPeer();
    } catch (e) {
      klog.error('call.onAccept', 'peer build failed', { err: e.message });
      this._teardown('couldn\'t set up audio (' + e.message + ')');
    }
  }
  ...
  async onSignal(msg) {
    ...
    if (!this.pc) {
      // PC was never built — happens if a stray signal arrives after
      // teardown, or before either side accepted. Drop it; the new
      // protocol guarantees pc exists by the time real signals arrive
      // (callee builds in accept(), caller builds in onAccept()).
      klog.warn('call.signal', 'no pc yet, dropping', { kind: payload.kind, call: this.callId, state: this.state });
      return;
    }
    // Perfect-negotiation pattern. Lets us add/remove tracks (e.g. screen
    // share) on either side without races. See:
    ...
    try {
      if (payload.kind === 'offer' || payload.kind === 'answer') {
        ...
        if (this._ignoreOffer) {
          klog.info('call.signal', 'glare ignored (impolite)', { kind: description.type });
          return;
        }
        ...
        catch (e) { if (!this._ignoreOffer) throw e; }
      }
    } catch (e) {
  ...
  async _buildPeer() {
    ...
    // Perfect-negotiation state. The "polite" peer rolls back on glare;
    // the impolite one wins. We designate the caller as polite (arbitrary
    // but consistent — both sides know this from msg.fromUserId checks
    // wouldn't help since the role is per-call, not per-user).
    ...
    // Perfect-negotiation pattern: createOffer + send whenever the
    // implementation says we need to renegotiate (initial track add,
    // screen-share track add/remove, etc).
...
async function openScreenShareModal() {
  const inElectron = !!(window.klar && window.klar.screen && window.klar.screen.sources);
  if (!inElectron) {
    // Browser path — just call straight through with current settings.
    const ss = state.settings.screenShare;
    await callMgr.startScreenShare({ height: ss.height, fps: ss.fps });
    return;
  }
