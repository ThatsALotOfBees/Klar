...
// The bootstrap (desktop/main.cjs) calls start() AFTER it has decided
// whether to use this bundled host or a freshly-downloaded one from
// GitHub. Module-level state above (helper fns, IPC handlers, app.on
// listeners) all run on require() — that's fine, they have no side
// effects until app is ready.
async function start() {
  ...
  sessionLog('INFO', 'app.config', 'resolved', {
    ...
    hostSource: module.filename, // tells us if we're bundled vs userData live
  });
  try {
    await createWindow();
